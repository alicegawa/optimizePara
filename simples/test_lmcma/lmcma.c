...
void LMCMA(int N, int lambda, int mu, double ccov, double xmin, double xmax, int nvectors,
	...
	int maxevals, enum FunctionId FuncId, int inseed, double* output, int printToFile, int sample_symmetry)
{
	// memory allocation
	// m*n
...
#pragma omp parallel for reduction(+:mueff)
	...
	int sign = 1; 
	for(i=0; i<lambda; i++) // O(lambda*m*n)
	{
	    if (sign == 1)
	    { 
		for( k=0; k<N; k++)	// O(n)
	    ...
	    //we should rewrite here using MPI and NEURON
...
void sepCMA(int N, int lambda, int mu, double ccov, double xmin, double xmax, int nvectors,
	int maxsteps, double cc, double val_target, double sigma, double c_s, double target_f, 
	int maxevals, enum FunctionId FuncId, int inseed, double* output, int printToFile)
{
	// memory allocation
	// m*n
    double* arx = (double *)malloc(sizeof(double) * N * lambda);
	double* arz = (double *)malloc(sizeof(double) * N * lambda);
	// n
	double* diagD = (double *)malloc(sizeof(double) * N * lambda);
	double* diagC = (double *)malloc(sizeof(double) * N * lambda);
	double* pc = (double *)malloc(sizeof(double) * N * lambda);
	double* ps = (double *)malloc(sizeof(double) * N * lambda);
	double* xmean = (double *)malloc(sizeof(double) * N * lambda);
	double* xold = (double *)malloc(sizeof(double) * N * lambda);
	// lambda, mu, nvectors
	double* weights = (double *)malloc(sizeof(double) * mu);	
	double* arfitness = (double *)malloc(sizeof(double) * lambda);
	int* arindex = (int *)malloc(sizeof(int) * lambda);
	...
	random_exit(&gt.ttime);
	free(arr_tmp);  free(weights); free(pc); free(xmean);
	free(xold); free(arz); free(arx); free(arfitness);
	free(arindex); free(diagD); free(diagC); free(ps);
	free_gt(&gt);
}
...
void runCMAmulambdaCholesky(int N,  int lambda, int mu, double sigma, double xmin, double xmax, double maxevals, double target_f, enum FunctionId FuncId, int verbose, int inseed, double* output, int printToFile)
{
// memory allocation
	// n*n
	...
	double* xmean = (double *)malloc(sizeof(double) * N);
	double* zmean = (double *)malloc(sizeof(double) * N);
	double* xold = (double *)malloc(sizeof(double) * N);
	double* tmp_vec = (double *)malloc(sizeof(double) * N);
	double* tmp_vec2 = (double *)malloc(sizeof(double) * N);
	// lambda, mu, nvectors
	double* weights = (double *)malloc(sizeof(double) * mu);	
	double* arfitness = (double *)malloc(sizeof(double) * lambda);
	int* arindex = (int *)malloc(sizeof(int) * lambda);
	...
	free(arr_tmp); free(weights); free(pc); free(xmean);
	free(xold); free(arz); free(arx); free(arfitness);
	free(arindex); free(tmp_vec); free(tmp_vec2); free(ps);
	free(A); free(Ainv); free(zmean);
	free_gt(&gt);
}
...
{	// OperationType: 0 - VectorScalar; 1 - MatrixVector; 2 - RandomScalar;	3 - FunctionEvaluation
	...
	{
		double val = MyFunc((enum FunctionId)FuncId,N,V1,&gt);
	}
	...
	while (run == 1)
	{
		...
		for (iEval=0; iEval<nEvals; iEval = iEval + 1)
		{
			if (OperationType == 0)
			{
			...
			if (OperationType == 3)
			{
				val = MyFunc((enum FunctionId)FuncId,N,V1,&gt);
			}
		}
	...
	if (OperationType == 1)
	    free(M);
	free(V1); free(V2);
	free(gt.func_tempdata);
	free(gt.x_tempdata); free(gt.rotmatrix); free(gt.func_shiftxi);
	return timePerEval;
}
...
int main()
{
	...
	enum FunctionId FuncId = (enum FunctionId)((int)2);	//	fitness function ID, use pointer to function if needed
