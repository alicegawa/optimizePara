...
int main(int argc, char **argv){
    ...
    int gen_restart[] = { 3, 8, 13, 18, 23, 28, 33, 38, 43, 48, 50, 100, 150};/* temporary setting */
    double *restartX, *restartSigma;
    double restartSigma_defaults = 10.0;//2.0;

    /* distributed pop update*/
    double *rgD, *x_mean, sigma, sigmasquare_div; 
    random_t rand_box;
    bcast_params rand_box_temp;
...
//num_of_pop_per_procs = num_of_pop_per_split / num_of_procs_nrn;
	...
	initialSigma[i] = 40;//100;//(10.0 - 0.0) * 0.25;
	...
	rand_box = evo.rand;
	/* rand_box_temp.seed_and_rand[0] = rand_box.startseed; */
        /* rand_box_temp.seed_and_rand[1] = rand_box.aktseed; */
        /* rand_box_temp.seed_and_rand[2] = rand_box.aktrand; */
        /* for(i=0;i<32;++i){ */
	/*   rand_box_temp.seed_and_rand[i + 3] = rand_box.rgrand[i]; */
        /* } */
        /* rand_box_temp.flgstored = rand_box.flgstored; */
        /* rand_box_temp.hold = rand_box.hold; */
	for(i=0;i<mu;++i){
    ...
    /* section[1]=1; */
    /* section[2]=1; */
    /* MPI_Datatype partype1, type[3]; */
    /* type[0] = MPI_LONG; */
    /* type[1] = MPI_SHORT; */
    /* type[2] = MPI_DOUBLE; */
    /* MPI_Aint byte[3], extent0, extent1; */
    /* byte[0] = 0; */
    /* MPI_Type_extent(MPI_LONG, &extent0); */
    /* byte[1] = 35 * extent0; */
    /* MPI_Type_extent(MPI_SHORT, &extent1); */
    /* byte[2] = 36 * extent0 + extent1; */
    /* MPI_Type_struct(3, section, byte, type, &partype1); */
    /* MPI_Type_commit(&partype1); */
    ...
    //MPI_Bcast(&rand_box_temp, 1, partype1, root_process_main, firstTimeWorld);
    ...
    /*   rand_box.aktseed = rand_box_temp.seed_and_rand[1]; */
    /*   rand_box.aktrand = rand_box_temp.seed_and_rand[2]; */
    /*   for(i=0;i<32;++i){ */
    /* 	rand_box.rgrand[i] = rand_box_temp.seed_and_rand[3+i]; */
    /*   } */
    /*   rand_box.flgstored = rand_box_temp.flgstored; */
    /*   rand_box.hold = rand_box_temp.hold; */
    /*   if(rand_box.hold!=0) */
    /* 	rand_box.hold = 0.0; */
    /* } */
	...
	if(I_AM_ROOT_IN_SPLIT){
	    ...
	    if(I_AM_ROOT_IN_MAIN && loop_count==0){
	      t_middle = MPI_Wtime();
	      printf("start 1st time NEURON simulation is %lf\n", t_middle - t_start0);
	    }
