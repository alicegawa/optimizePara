...
/* modified by akihikogoto 2014.05.01 */
void readpara_init (readpara_t *, int dim, int seed,  const double * xstart, 
                    const double * sigma, int lambda, int mu, int maxFunEvals, int maxStopIter, const char * filename);
/* void readpara_init (readpara_t *, int dim, int seed,  const double * xstart,  */
...
static void Adapt_C2_dist(cmaes_t *t, int hsig, double *sum_cov_temp);
...
cmaes_init(cmaes_t *t, /* "this" */
		   ...
		   int lambda, 
		   int mu, 
		   int maxFunEvals, /* default -1 (added by akihikogoto) */
		   int maxStopIter, /* default -1 (added by akihikogoto) */
		   const char *input_parameter_filename) 
{
  ...
  readpara_init (&t->sp, dimension, inseed, inxstart, inrgstddev, 
				 lambda, mu, maxFunEvals, maxStopIter, input_parameter_filename);
  ...
  for (dtest = 1.; dtest && dtest < 1.1 * dtest; dtest *= 2.) 
    if (dtest == dtest + 1.)
      break;
  t->dMaxSignifKond = 1e23; // dtest / 1000.; /* not sure whether this is really save, 100 does not work well enough */
  // t->dMaxSignifKond = 1e16; // dtest / 1000.; /* not sure whether this is really save, 100 does not work well enough */
...
cmaes_exit(cmaes_t *t)
{
  ...
  if(t->rgpc != NULL) {free( t->rgpc); t->rgpc=NULL;}
  if(t->rgps != NULL) {free( t->rgps); t->rgps=NULL;}
  if(t->rgdTmp != NULL) {free( t->rgdTmp); t->rgdTmp=NULL;}
  if(t->rgBDz != NULL) {free( t->rgBDz); t->rgBDz=NULL;}
  if(t->rgxmean != NULL) {free( --t->rgxmean); t->rgxmean=NULL;}
  if(t->rgxold != NULL) {free( --t->rgxold);  t->rgxold=NULL;}
  if(t->rgxbestever != NULL) {free( --t->rgxbestever);  t->rgxbestever=NULL;}
  if(t->rgout != NULL) {free( --t->rgout);  t->rgout=NULL;}
  if(t->rgD != NULL) {free( t->rgD); t->rgD=NULL;}
  for (i = 0; i < N; ++i) {
    if(t->C[i] != NULL) {free( t->C[i]); t->C[i]=NULL;}
    if(t->B[i] != NULL) {free( t->B[i]); t->B[i]=NULL;}
  }
  for (i = 0; i < t->sp.lambda; ++i) 
    if(t->rgrgx[i] != NULL) {free( --t->rgrgx[i]); t->rgrgx[i]=NULL;}
  if(t->rgrgx != NULL) {free( t->rgrgx);  t->rgrgx=NULL;}
  if(t->C != NULL) {free( t->C); t->C=NULL;}
  if(t->B != NULL) {  free( t->B); t->B=NULL;}
  if(t->index != NULL) {  free( t->index); t->index=NULL;}
  if(t->publicFitness != NULL) {  free( t->publicFitness); t->publicFitness=NULL;}
  if(t->rgFuncValue != NULL) {  free( --t->rgFuncValue); t->rgFuncValue=NULL;}
  if(t->arFuncValueHist != NULL) {  free( --t->arFuncValueHist); t->arFuncValueHist=NULL;}
...
/* --------------------------------------------------------- */
double * const * 
cmaes_SamplePopulation(cmaes_t *t) /* ub and lb are added by akihiko goto */
{
...
#pragma omp parallel for reduction(+:sum)
	  for (iNk = 0; iNk < t->sp.mu; ++iNk) {
		  sum += t->sp.weights[iNk] * t->rgrgx[t->index[iNk]][i];
	  }
	  t->rgxmean[i] = sum;
	  t->rgBDz[i] = sqrt(t->sp.mueff)*(t->rgxmean[i] - t->rgxold[i])/t->sigma;
  }
...
/*dist ver. added by fukuda 28 July. 2016*/
/* --------------------------------------------------------- */
/* --------------------------------------------------------- */
double *
cmaes_UpdateDistribution_dist( cmaes_t *t, const double *rgFunVal, double *sum_temp, double *sum_cov_temp)
{
  int i, j, iNk, hsig, N=t->sp.N;
  int flgdiag = ((t->sp.diagonalCov == 1) || (t->sp.diagonalCov >= t->gen)); 
  double sum; 
  ...
  Adapt_C2_dist(t, hsig, sum_cov_temp);
  ...
  while (key < keyend)
    {
     ...
     /* covariance matrix (added by akihiko goto 2014.03.04)*/
      if (strncmp(key, "lineC", 1) == 0) 
        {
          int j;
          for (i = 0; i < N; ++i) {
			  for (j = 0; j <= i; ++j) {
				  fprintf(fp, "%g", t->C[i][j]);
				  if (j < N-1) {
					  fprintf(fp, "%c", '\t');
				  }
			  }
...
cmaes_Get( cmaes_t *t, char const *s)
{
  ...
  else if (strncmp(s, "fmedian", 7) == 0) {
	  return(t->rgFuncValue[t->index[(int)(t->sp.lambda/2)]]);
  }
  else if (strncmp(s, "fworst", 6) == 0) {
	  return(t->rgFuncValue[t->index[t->sp.lambda-1]]);
  }
...
/* --- added by akihiko goto (2013.01.11)--- */
const double * 
cmaes_GetXPtr( cmaes_t *t, unsigned int idx)
{
	if ( idx < t->sp.lambda)
		return(t->rgrgx[t->index[idx]]);
...
{
	if(t->rgrand!=NULL) { free( t->rgrand); t->rgrand = NULL; }
  // free( t->rgrand);
}
               ...
               const char * filename)
{
  ...
  t->stopMaxFunEvals = maxFunEvals;//-1;
  t->stopMaxIter = maxStopIter;//-1;
  t->facmaxeval = 1; 
