...
int main(int argc, char** argv){
    ...
    int root_porcess_main = 0, root_process_split = 0, root_process_spawn = 0;
    int color, key;

    FILE *fp;
    char specials[] = "special";
    char *option_mpi[] = {"-mpi","-nobanner","./test.hoc",NULL};
    ...
    scatter2 = (double *)malloc(sizeof(double)*3);
	...
	if(split_myid==0){
	    ...
	    MPI_Scatter(scatter1, 3, MPI_DOUBLE, scatter2, 3, MPI_DOUBLE, 0, nrn_comm);
	    printf("end of mpi scatter\n");
	    //MPI_Gather(gather1, 3, MPI_DOUBLE, gather2, 3, MPI_DOUBLE, 0, nrn_comm);
